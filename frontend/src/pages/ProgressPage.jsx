...
function ProgressPage() {
  ...
  const handleSubmit = async () => {
    ...
    const teacherId =
      form.role === 'teacher' ? userId : form.friendId;

    const learnerId =
      form.role === 'teacher' ? form.friendId : userId;
  ...
  const markAsCompleted = async (sessionId) => {
    try {
      await axios.post(
        `http://localhost:5000/api/mark-complete/${sessionId}`
      );
  ...
  const upcoming = schedules.filter(
    s => new Date(s.scheduled_time) > now && !s.is_completed
  );
  ...
  return (
    <div className="progress-container">
      ...
      <p className="progress-subtitle">
        Schedule sessions and track your learning journey.
      </p>
      ...
      {showForm && (
        <div className="schedule-form">
          ...
          <select
            value={form.friendId}
            onChange={e =>
              setForm({ ...form, friendId: e.target.value })
            }
          >
